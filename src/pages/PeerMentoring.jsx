...
function PeerMentoring() {
  ...
  useEffect(() => {
    fetch(guideSrc)
      ...
      .then((t) => setHtml(t))
      .catch(() => setHtml('<div class="p-6">Could not load guide content.</div>'));
  }, [guideSrc]);

  return (
    <>
      <PageHero title="Peer Mentoring Toolkit" subtitle="Read the complete guide or download the toolkit PDF" />

      <section className="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
        <Card title="Toolkit">
          <p className="mb-4">The full Peer Mentoring toolkit is shown below. You can also open the standalone guide in a new tab or download the PDF.</p>
        ...
        <div className="mt-6 rounded-lg overflow-hidden border border-white/20 bg-white">
          <div className="prose max-w-none p-6" dangerouslySetInnerHTML={{ __html: html }} />
        </div>
      </section>
    </>
  );
