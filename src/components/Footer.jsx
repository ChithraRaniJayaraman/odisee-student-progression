...
function Footer() {
  return (
    <footer className="mt-20 border-t border-white/55 bg-gradient-to-b from-section/70 to-main/90 text-royal">
      <div className="mx-auto grid max-w-7xl gap-8 px-4 py-10 sm:px-6 lg:grid-cols-2 lg:px-8">
        ...
        <div>
          <h3 className="font-semibold text-royal">Navigation</h3>
          <ul className="mt-3 grid grid-cols-2 gap-2 text-sm">
            {navItems.map((item) => (
              <li key={item.path}>
                <Link to={item.path} className="text-body/90 transition hover:text-royal">
                  {item.label}
                </Link>
              </li>
            ))}
